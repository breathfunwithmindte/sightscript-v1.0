...
module.exports = class Tokenizer {
  ...
  tokenize () 
  {
    ...
    script = this.#regexReplace(script, "\\/\\*\\*((.|\n|\r)*?)\\*/", { idName: "__COMMENT__" });
    script = this.#regexReplace(script, `"((.|\n|\r)*?)"`, { idName: "__STRING__" });
    script = this.#regexReplace(script, `'((.|\n|\r)*?)'`, { idName: "__STRING__" });
    script = this.#regexReplace(script, "`((.|\n|\r)*?)`", { idName: "__TSTRING__" });
    script = this.#regexReplace(script, "<:((.|\n|\r)*?):>", { idName: "__GROUP__" });
    script = this.#regexReplace(script, "\\{((.|\n|\r)*?)\\}", { idName: "__CODE__" });
   ...
   * @doc
   * removing the comments that starts and finish with /** * {__without space__} /
   * also split lines and removing white spaces (trim them) after that join the lines with \n
   */
  #removeComments (script) 
  {
    return this.#regexReplace(script, "\\/\\*\\*((.|\n)*?)\\*\\/", { replaceWith: "\n", store: false })
               .split(/\r?\n/)
               .map(i => i.trim())
               .filter(f => f !== "")
               .join("\n")
