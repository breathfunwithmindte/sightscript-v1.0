...
module.exports = class ParserStatement extends ParserObjArr {
  ...
  parse_expression (codeblock_string, tokens)
  {
    ...
    Expression.map(exp => { expression_line = expression_line.replaceAll(exp, exp + " \n") }) 
  ...
  #find_expression_line_token (expression_line, tokens)
  {
    ...
    let current_expression_line = expression_line;
    Expression.map(i => {
      if(expression_line.endsWith(i)) {
        current_token = i;
        current_expression_line = expression_line.substring(0, expression_line.length - i.length).trim();
      }
    })
