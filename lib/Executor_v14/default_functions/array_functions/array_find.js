...
module.exports = new FunctionExe("find", async function (executor, scope_state, current) {
  let [actual_array, callback] = await executor.get_anonumous_props(current.prop, scope_state);
  if(actual_array instanceof Array === false) throw new Error("Array is required for the function find.");
